# -*- coding: utf-8 -*-
"""
Created on Tue Feb  1 12:30:20 2022
...
import tqdm
import numpy as np
from utils_camvid import poly_lr_scheduler
from utils_camvid import reverse_one_hot,compute_global_accuracy,per_class_iu,fast_hist
    ...
    with torch.no_grad():
        ...
        for i,(data,label) in enumerate(dataloader):
            if torch.cuda.is_available() and args.use_gpu:
                data=data.cuda()
                label=label.cuda()
            predict=model(data).squeeze().cpu()
...
def train(args,model,optim,dataloader_train,dataloader_val):
    ...
    for epoch in range(args.num_epochs):
        lr=poly_lr_scheduler(optim, args.learning_rate, iters=epoch,max_iter=args.num_epochs)
        model.train()
        ...
        for i,(data,label) in enumerate(dataloader_train):
            if torch.cuda.is_available() and args.use_gpu:
                data=data.cuda()
                label=label.cuda()
            output,output_supp1,output_supp2=model(data)
            loss1=loss_func(output,label)
            loss2=loss_func(output_supp1,label)
            loss3=loss_func(output_supp2,label)
        ...
        if epoch%args.checkpoint_step==0 and epoch!=0:
            torch.save(model.module.state_dict(),'latest_dice_loss.pth')
        if epoch%args.validation_step==0:
            precision,miou=val(args,model,dataloader_val)
            if miou>max_iou:
                max_iou=miou
                torch.save(model.module.state_dict(),'best_dice_loss.pth')
